...
class AdminLoginController extends MY_Controller
{
    ...
    {
        $this->load->view('common/AdminLogin',$this->data);
    }
    ...
    public function ajaxAction_login()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $this->AccountInformation = $this->Login_Model->getAdminEmployeeByUserName($this->UserName);
            if (!empty($this->AccountInformation)) {
                ...
                if ($this->PasswordComparison) {
                    if ($this->AccountInformation['status'] == 'active') {
                        $this->logged_in_sess = array('adminEmployeeId'=>$this->AccountInformation['adminEmployeeId']);
