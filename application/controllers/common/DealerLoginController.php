...
class DealerLoginController extends MY_Controller
{
    private $UserName;
    private $password;
    ...
    {
        $this->load->view('common/dealerLogin',$this->data);
    }
    ...
    public function ajaxAction_login()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $this->AccountInformation = $this->Login_Model->getDealerEmployeeByUserName($this->UserName);
            if (!empty($this->AccountInformation)) {
                ...
                if ($this->PasswordComparison) {
                    if ($this->AccountInformation['status'] == 'active') {
                        $this->logged_in_sess = array('dealerEmployeeId'=>$this->AccountInformation['dealerEmployeeId']);
